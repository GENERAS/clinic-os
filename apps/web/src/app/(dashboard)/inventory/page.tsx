...
import type { InventoryItemSummary } from "@/features/inventory/types"
...
export default function InventoryPage() {
  ...
  const handleAdjust = async (values: StockAdjustmentFormValues) => {
    if (!clinicId || !user || !adjustItemId) return
    try {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Inventory"
        description={`${total} item${total !== 1 ? "s" : ""} tracked`}
      >
        <Link
          href="/inventory/new"
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid gap-3 md:hidden">
            {items.map((item) => (
              <InventoryCard key={item.id} item={item} onAdjust={setAdjustItemId} />
            ))}
          </div>
