...
import { ArrowLeft, Loader2, Calendar, Clock, User, Stethoscope, FileText, Search } from "lucide-react"
import Link from "next/link"
import { PageHeader } from "@/components/shared/page-header"
import { SectionCard } from "@/components/shared/section-card"
import { useAuth } from "@/features/auth/hooks/use-auth"
import { getAppointmentService } from "@/features/appointments/services/appointment.service"
import { createAppointmentSchema, type CreateAppointmentFormValues } from "@/features/appointments/schemas"
import { PatientSearchBar } from "@/features/patients/components/patient-search-bar"
import type { PatientSearchResult } from "@/features/patients/types"
          ...
          <div className="space-y-4">
            <div>
              ...
              {errors.appointment_date && (
                <p className="mt-1 text-xs text-destructive">{errors.appointment_date.message}</p>
              )}
            </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                id="reason"
                type="text"
                placeholder="e.g. Routine checkup, fever, prescription refill"
                {...register("reason")}
                disabled={saving}
                className="w-full rounded-lg border bg-background px-3 py-2 text-sm disabled:opacity-60"
              />
            ...
            <div>
              ...
              <textarea
                id="notes"
                rows={3}
                placeholder="Any additional notes (not visible to patient)"
                {...register("notes")}
                disabled={saving}
                className="w-full rounded-lg border bg-background px-3 py-2 text-sm disabled:opacity-60"
              />
