...
import { appointmentNoteSchema, type AppointmentNoteFormValues } from "../schemas"
import type { AppointmentNoteWithAuthor } from "../types"

interface AppointmentNotesProps {
  notes: AppointmentNoteWithAuthor[]
  onAddNote: (content: string) => Promise<void>
}

export function AppointmentNotes({ notes, onAddNote }: AppointmentNotesProps) {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<AppointmentNoteFormValues>({
    resolver: zodResolver(appointmentNoteSchema),
  })

  const onSubmit = async (values: AppointmentNoteFormValues) => {
    setSaving(true)
    try {
