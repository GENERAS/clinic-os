...
import type { ClinicNotificationSetting } from "@/types/database"
import type { NotificationSettingsFormValues } from "../schemas/notification-settings.schema"

interface ClinicNotificationFormProps {
  settings: ClinicNotificationSetting | null
  isOwner: boolean
  onSave: (values: NotificationSettingsFormValues) => Promise<void>
}

const TOGGLES: { key: keyof NotificationSettingsFormValues; label: string; description: string }[] = [
  {
...
export function ClinicNotificationForm({ settings, isOwner, onSave }: ClinicNotificationFormProps) {
  ...
  const handleToggle = async (key: keyof NotificationSettingsFormValues) => {
  ...
  return (
    <SectionCard
      ...
      description="Configure which notifications your clinic receives"
      actions={
        !isOwner && (
          ...
          {TOGGLES.map(({ key, label, description }) => (
            <div key={key} className="flex items-center justify-between rounded-lg border bg-background p-4">
              ...
              <button
                ...
                onClick={() => isOwner && handleToggle(key)}
