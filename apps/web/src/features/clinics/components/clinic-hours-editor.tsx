...
import type { ClinicOperatingHour } from "@/types/database"
...
interface ClinicHoursEditorProps {
  hours: ClinicOperatingHour[]
  isOwner: boolean
  onSave: (hours: HoursEntry[]) => Promise<void>
}

function getEmptyHours(): HoursEntry[] {
...
function mergeHours(existing: ClinicOperatingHour[]): HoursEntry[] {
  const map = new Map<number, HoursEntry>()
...
export function ClinicHoursEditor({ hours, isOwner, onSave }: ClinicHoursEditorProps) {
  const [entries, setEntries] = useState<HoursEntry[]>(() => mergeHours(hours))
  ...
  const updateEntry = (index: number, partial: Partial<HoursEntry>) => {
    setEntries((prev) => {
      const next = [...prev]
      next[index] = { ...next[index], ...partial } as HoursEntry
      return next
    })
  }
  ...
  return (
    <SectionCard
      ...
      description="Set your clinic's weekly operating schedule"
      actions={
        !isOwner && (
      ...
      {entries.length === 0 ? (
        ...
        <div className="divide-y">
          {entries.map((entry, index) => (
            <div key={entry.day_of_week} className="flex flex-col gap-3 py-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                {isOwner && (
                  <label className="relative inline-flex cursor-pointer items-center">
                    <input
                      ...
                      onChange={(e) => updateEntry(index, { is_open: e.target.checked })}
                      disabled={!isOwner}
              ...
              {entry.is_open ? (
                <div className="flex items-center gap-2">
                  <input
                    ...
                    onChange={(e) => updateEntry(index, { open_time: e.target.value })}
                    disabled={!isOwner}
                    className="rounded-lg border bg-background px-3 py-1.5 text-sm disabled:opacity-60"
                  />
                  <span className="text-sm text-muted-foreground">to</span>
                  <input
                    type="time"
                    value={entry.close_time}
                    onChange={(e) => updateEntry(index, { close_time: e.target.value })}
                    disabled={!isOwner}
                    className="rounded-lg border bg-background px-3 py-1.5 text-sm disabled:opacity-60"
                  />
                </div>
