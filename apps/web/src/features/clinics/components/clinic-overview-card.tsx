...
const statusConfig: Record<ClinicStatus, { label: string; className: string }> = {
...
export function ClinicOverviewCard({ name, logoUrl, status, city, isOpenNow, className }: ClinicOverviewCardProps) {
  const statusInfo = statusConfig[status]

  return (
    <div className={cn("flex items-center gap-4 rounded-xl border bg-card p-4 shadow-sm", className)}>
      {logoUrl ? (
        <img src={logoUrl} alt={name} className="size-14 rounded-lg border object-cover" />
      ) : (
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 text-sm text-muted-foreground">
          ...
          {isOpenNow !== undefined && (
            <span className={cn("flex items-center gap-1", isOpenNow ? "text-green-600" : "text-muted-foreground")}>
              <Circle className="size-2 fill-current" />
