...
import { clinicLocationSchema, type ClinicLocationFormValues } from "../schemas/clinic-profile.schema"
import type { Clinic } from "@/types/database"

interface ClinicLocationFormProps {
  clinic: Clinic
  isOwner: boolean
  onSave: (values: ClinicLocationFormValues) => Promise<void>
}

export function ClinicLocationForm({ clinic, isOwner, onSave }: ClinicLocationFormProps) {
  ...
  const onSubmit = async (values: ClinicLocationFormValues) => {
    setSaving(true)
    setSuccess(false)
    try {
      await onSave(values)
  ...
  return (
    <SectionCard
      ...
      description="Physical location information. Multi-branch support coming in a future update."
      actions={
        !isOwner && (
