...
export function PatientAppointmentsList({ clinicId, patientId }: PatientAppointmentsListProps) {
  ...
  if (appointments.length === 0) {
    return (
      <p className="py-8 text-center text-sm text-muted-foreground">No appointment history</p>
    )
  }
  ...
  return (
    <div className="space-y-3">
      {appointments.map((apt) => {
        const doctor = apt.doctor as { id: string; full_name: string } | undefined
        return (
          <Link
          ...
          >
            <div className="flex size-9 items-center justify-center rounded-full bg-muted text-xs font-medium">
              {((apt.patient_name as string) || "?").charAt(0).toUpperCase()}
            </div>
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2">
                <Calendar className="size-3.5 text-muted-foreground" />
                <span className="text-sm font-medium">
                  {new Date((apt.appointment_date as string) + "T00:00:00").toLocaleDateString()}
                </span>
                <Clock className="size-3.5 text-muted-foreground" />
                <span className="text-xs text-muted-foreground">
                  {(apt.start_time as string)?.substring(0, 5)} — {(apt.end_time as string)?.substring(0, 5)}
                </span>
              </div>
            ...
            <AppointmentStatusBadge status={(apt.status as AppointmentStatus) || "scheduled"} />
          </Link>
        )
      })}
