...
export function StockAdjustmentDialog({ open, onClose, onSubmit }) {
      ...
      <div className="mx-4 w-full max-w-md rounded-xl bg-card p-6 shadow-lg" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit(handleFormSubmit)} className="mt-4 space-y-4">
          <div>
            ...
            <select id="type" {...register("type")} disabled={saving} className="w-full rounded-lg border bg-background px-3 py-2 text-sm disabled:opacity-60">
          ...
          <div>
            ...
            {selectedType === "adjustment" && (<p className="mt-1 text-xs text-muted-foreground">
          ...
          <div>
            ...
            <input id="reason" type="text" placeholder={selectedType === "stock_in" ? "e.g. Supplier delivery" :
            selectedType === "stock_out" ? "e.g. Dispensed to patient" :
                selectedType === "expired" ? "e.g. Expired batch" :
                    "e.g. Counting correction"} {...register("reason")} disabled={saving} className="w-full rounded-lg border bg-background px-3 py-2 text-sm disabled:opacity-60"/>
          </div>
