...
export function ReminderSettingsCard({ settings, onSave }) {
    ...
    const { register, handleSubmit, reset, watch, formState: { errors, isDirty }, } = useForm({
        ...
        values: settings || {
            whatsapp_reminders_enabled: false, reminder_hours_before: 24,
            sms_reminders_enabled: false, sms_reminder_hours_before: 24,
        },
    });
    const whatsappEnabled = watch("whatsapp_reminders_enabled");
    const smsEnabled = watch("sms_reminders_enabled");
    ...
    return (<div className="rounded-xl border border-slate-200 bg-white p-4 shadow-sm">
      ...
      <p className="mt-1 text-xs text-slate-500">Configure automated WhatsApp and SMS reminders for upcoming appointments</p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="mt-4 space-y-6">
        ...
        <div className="border-t border-slate-100 pt-4 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <label className="relative inline-flex cursor-pointer items-center">
              <input type="checkbox" className="peer sr-only" {...register("sms_reminders_enabled")}/>
              <div className="h-6 w-11 rounded-full bg-slate-200 after:absolute after:left-[2px] after:top-[2px] after:h-5 after:w-5 after:rounded-full after:border after:border-slate-300 after:bg-white after:transition-all after:content-[''] peer-checked:bg-teal-600 peer-checked:after:translate-x-full peer-checked:after:border-white"/>
            </label>
          </div>
          ...
          {smsEnabled && (<div>
              ...
              <select {...register("sms_reminder_hours_before", { valueAsNumber: true })} className="mt-1 block w-full rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900">
                {REMINDER_HOURS_OPTIONS.map((hours) => (<option key={hours} value={hours}>
                    {hours} hour{hours > 1 ? "s" : ""} before
                  </option>))}
              </select>
              {errors.sms_reminder_hours_before && (<p className="mt-1 text-xs text-red-500">{errors.sms_reminder_hours_before.message}</p>)}
            </div>)}

          {errors.sms_reminders_enabled && (<p className="text-xs text-red-500">{errors.sms_reminders_enabled.message}</p>)}
        </div>
