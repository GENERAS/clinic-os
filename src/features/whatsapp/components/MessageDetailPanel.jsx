    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <p className="text-sm text-slate-900">{message.patient_name || "N/A"}</p>
        </div>
        <div>
          <label className="text-xs font-medium text-slate-500">Phone</label>
          <p className="text-sm text-slate-900">{message.phone_number}</p>
        </div>
        <div>
          <label className="text-xs font-medium text-slate-500">Type</label>
          <p className="text-sm text-slate-900">{MESSAGE_TYPE_LABELS[message.message_type]}</p>
        </div>
        <div>
          <label className="text-xs font-medium text-slate-500">Template</label>
          <p className="text-sm text-slate-900">{message.message_template}</p>
        </div>
        <div>
          <label className="text-xs font-medium text-slate-500">Provider</label>
          <p className="text-sm text-slate-900">{message.provider}</p>
        </div>
        <div>
          <label className="text-xs font-medium text-slate-500">Provider Message ID</label>
          <p className="text-sm text-slate-900">{message.provider_message_id || "—"}</p>
        </div>
      ...
      <div>
        <label className="text-xs font-medium text-slate-500">Status Timeline</label>
        <div className="mt-2 space-y-3">
          {statusHistory.map((item) => (<div key={item.label} className="flex items-center gap-3">
              <div className={`h-2.5 w-2.5 rounded-full ${item.active ? "bg-teal-500" : "bg-slate-200"}`}/>
              <div>
                <p className={`text-sm ${item.active ? "text-slate-900" : "text-slate-400"}`}>
                  {item.label}
                </p>
                {item.time && (<p className="text-xs text-slate-500">
          ...
          {message.status === "failed" && (<div className="flex items-center gap-3">
              ...
              <div>
                ...
                {message.failed_at && (<p className="text-xs text-slate-500">
