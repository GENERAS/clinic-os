...
}, { message: "End time must be after start time", path: ["end_time"] }).refine((data) => {
...
export const rescheduleAppointmentSchema = z.object({
    ...
    start_time: z.string().min(1, "Start time is required"),
    end_time: z.string().min(1, "End time is required"),
}).refine((data) => data.start_time < data.end_time, { message: "End time must be after start time", path: ["end_time"] });
