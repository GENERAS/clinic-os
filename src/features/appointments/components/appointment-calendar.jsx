...
export function AppointmentCalendar({ appointments, currentDate, view, onDateChange, onViewChange, onAppointmentClick, }) {
    ...
    if (view === "month") {
        return (<MonthView currentDate={currentDate} appointments={appointments} navigate={navigate} headerLabel={headerLabel} goToday={goToday} onViewChange={onViewChange} onDateChange={onDateChange} onAppointmentClick={onAppointmentClick} getAppointmentsForDay={getAppointmentsForDay}/>);
    }
    if (view === "week") {
        return (<WeekView currentDate={currentDate} appointments={appointments} navigate={navigate} headerLabel={headerLabel} goToday={goToday} onViewChange={onViewChange} onDateChange={onDateChange} onAppointmentClick={onAppointmentClick} getAppointmentsForDay={getAppointmentsForDay}/>);
    }
    return (<DayView currentDate={currentDate} appointments={appointments} navigate={navigate} headerLabel={headerLabel} goToday={goToday} onViewChange={onViewChange} onDateChange={onDateChange} onAppointmentClick={onAppointmentClick} getAppointmentsForDay={getAppointmentsForDay}/>);
}
...
function MonthView({ currentDate, navigate, headerLabel, goToday, onViewChange, onDateChange, onAppointmentClick, getAppointmentsForDay, }) {
    const days = useMemo(() => {
      ...
      <div className="rounded-xl border">
        ...
        <div className="grid grid-cols-7">
          {days.map((day) => {
            ...
            return (<button key={day.toISOString()} onClick={() => {
                    onDateChange(day);
                    onViewChange("day");
                }} className={`min-h-[80px] border-b border-r p-1.5 text-left transition-colors hover:bg-muted/30 ${!isCurrentMonth ? "opacity-40" : ""} ${today ? "bg-primary/5" : ""}`}>
                ...
                <div className="mt-1 space-y-0.5">
                  {dayApts.slice(0, 3).map((apt) => (<div key={apt.id} onClick={(e) => {
                        e.stopPropagation();
                        onAppointmentClick(apt.id);
                    }} className={`truncate rounded px-1 py-0.5 text-[10px] font-medium ${apt.status === "completed"
...
function WeekView({ currentDate, appointments, navigate, headerLabel, goToday, onViewChange, onDateChange, onAppointmentClick, getAppointmentsForDay, }) {
    const days = useMemo(() => {
        const weekStart = startOfWeek(currentDate, { weekStartsOn: 1 });
        const weekEnd = endOfWeek(currentDate, { weekStartsOn: 1 });
        ...
        <div className="min-w-[700px]">
          ...
          <div className="grid grid-cols-[60px_repeat(7,1fr)]">
            {hours.map((hour) => (<>
                ...
                return (<div key={`${day.toISOString()}-${hour}`} className="min-h-[48px] border-b border-r p-0.5">
                      {dayApts.map((apt) => (<button key={apt.id} onClick={() => onAppointmentClick(apt.id)} className={`w-full truncate rounded px-1 py-0.5 text-[10px] font-medium text-left ${apt.status === "completed"
...
function DayView({ currentDate, appointments, navigate, headerLabel, goToday, onViewChange, onAppointmentClick, getAppointmentsForDay, }) {
    ...
    return (<div className="space-y-4">
      ...
      <div className="rounded-xl border">
        {dayApts.length === 0 ? (<div className="flex flex-col items-center gap-2 py-12 text-center">
            ...
            {dayApts.map((apt) => (<button key={apt.id} onClick={() => onAppointmentClick(apt.id)} className="flex w-full items-center gap-4 px-4 py-3 text-left transition-colors hover:bg-muted/30">
