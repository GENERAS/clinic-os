...
export default function AppointmentDetailPage() {
    ...
    useEffect(() => {
        ...
        const channel = supabase
            ...
            .on("postgres_changes", { event: "UPDATE", schema: "public", table: "appointments", filter: `id=eq.${id}` }, async () => {
                const data = await service.getAppointment(clinicId, id);
                if (data) setAppointment(data);
            })
            .subscribe((s) => {
                setRealtimeStatus(s === "SUBSCRIBED" ? "connected" : "error");
            });
    ...
    return (<div className="mx-auto max-w-3xl space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-xl border p-4">
        ...
        <QuickActions appointmentId={appointment.id} currentStatus={appointment.status} onAction={handleStatusChange}/>
        <button onClick={() => setShowReschedule(true)} className="inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium hover:bg-muted">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <SectionCard title="Patient" icon={<User className="size-4"/>}>
          <div className="space-y-2">
            <div>
              {appointment.patient_id ? (<Link to={`/patients/${appointment.patient_id}`} className="text-sm font-medium text-primary hover:underline">
                  {appointment.patient_name}
                </Link>) : (<p className="text-sm font-medium">{appointment.patient_name}</p>)}
              {appointment.patient_phone && (<p className="text-xs text-muted-foreground">{appointment.patient_phone}</p>)}
            </div>
          </div>
        </SectionCard>
      ...
      {/* Reason */}
      {appointment.reason && (<SectionCard title="Reason for Visit" icon={<FileText className="size-4"/>}>
      ...
      {/* Reschedule Dialog */}
      <RescheduleDialog open={showReschedule} onClose={() => setShowReschedule(false)} onReschedule={handleReschedule} currentDate={appointment.appointment_date} currentStartTime={appointment.start_time} currentEndTime={appointment.end_time}/>
    </div>);
}
