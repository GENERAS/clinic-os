...
export default function NewPatientPage() {
    ...
    const onSubmit = async (values) => {
        ...
        setSaving(true);
        try {
            const patient = await getPatientService().createPatient(clinicId, {
            ...
            if (existsMatch) {
                ...
                navigate(`/patients/${existsMatch[1]}`);
            }
            else {
                toast.error(message);
            }
    ...
    return (<div className="mx-auto max-w-2xl space-y-6">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <SectionCard title="Basic Information" icon={<User className="size-4"/>}>
          <div className="space-y-4">
            <div>
              ...
              <input id="full_name" type="text" placeholder="Enter patient full name" {...register("full_name")} disabled={saving} className="w-full rounded-lg border bg-background px-3 py-2 text-sm disabled:opacity-60"/>
            ...
            <div>
              ...
              <input id="phone" type="tel" placeholder="+250 7XX XXX XXX" {...register("phone")} disabled={saving} className="w-full rounded-lg border bg-background px-3 py-2 text-sm disabled:opacity-60"/>
            ...
            <div>
              ...
              <input id="email" type="email" placeholder="patient@example.com" {...register("email")} disabled={saving} className="w-full rounded-lg border bg-background px-3 py-2 text-sm disabled:opacity-60"/>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <select id="gender" {...register("gender")} disabled={saving} className="w-full rounded-lg border bg-background px-3 py-2 text-sm disabled:opacity-60">
        ...
        <SectionCard title="Emergency Contact">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input id="emergency_contact_name" type="text" placeholder="Emergency contact name" {...register("emergency_contact_name")} disabled={saving} className="w-full rounded-lg border bg-background px-3 py-2 text-sm disabled:opacity-60"/>
            </div>
            <div>
              <label htmlFor="emergency_contact_phone" className="mb-1.5 block text-sm font-medium">
                Contact Phone
              </label>
              <input id="emergency_contact_phone" type="tel" placeholder="Emergency contact phone" {...register("emergency_contact_phone")} disabled={saving} className="w-full rounded-lg border bg-background px-3 py-2 text-sm disabled:opacity-60"/>
            </div>
          </div>
        </SectionCard>
        ...
        <SectionCard title="Additional Notes">
          <div>
            <textarea id="notes" rows={3} placeholder="Any additional notes about the patient" {...register("notes")} disabled={saving} className="w-full rounded-lg border bg-background px-3 py-2 text-sm disabled:opacity-60"/>
          </div>
        </SectionCard>

        {/* Submit */}
        <div className="flex justify-end gap-3">
          <Link to="/patients" className="rounded-lg border bg-background px-4 py-2 text-sm font-medium hover:bg-muted">
            Cancel
          </Link>
